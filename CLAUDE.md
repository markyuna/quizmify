...
- **Spending**: `src/lib/neurons/costs.ts` holds `NEURON_UNLOCK_COSTS`, currently one entry — unlocking Puzzle du Jour costs 50 Neurons. Kept separate from `ALL_GAMES` (`src/lib/games/allGames.ts`), which is presentation-layer config for listing games; the Neuron price a `pro-neuron` entry displays is imported from here. Expect more `gameKey` entries here as more Neuron-unlockable games ship.
