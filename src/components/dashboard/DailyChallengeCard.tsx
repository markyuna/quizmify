...
import { Flame, ArrowRight, CheckCircle2, Users } from "lucide-react";
...
export default async function DailyChallengeCard({ userId }: { userId: string }) {
      ...
      <CardContent className="relative z-10 p-4 sm:p-6">
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0 space-y-2">
            ...
            <p className="truncate text-lg font-bold text-foreground">{challenge.topic}</p>
        ...
        >
          {existingAttempt ? (
            <>
              ...
              {t("viewResult", { score: existingAttempt.score })}
            </>
          ) : (
            <>
              {t("playNow")}
