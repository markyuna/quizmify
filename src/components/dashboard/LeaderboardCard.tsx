...
import { getUserGlobalRank } from "@/lib/leaderboard";

export default async function LeaderboardCard({ userId }: { userId: string }) {
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0 space-y-2">
            ...
            <p className="text-lg font-bold text-foreground">
              {rank !== null ? t("yourRank", { rank }) : t("notRankedGlobal")}
            </p>
          </div>
          ...
          <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl border border-white/10 bg-white/60 shadow-sm backdrop-blur-xl dark:bg-white/5">
            <Trophy className="h-5 w-5 text-violet-500 dark:text-violet-300" />
          </div>
        </div>

        <Link
          href="/leaderboard"
          className="mt-4 inline-flex items-center gap-2 rounded-2xl bg-gradient-to-r from-violet-600 to-cyan-500 px-4 py-2.5 text-sm font-bold text-white shadow-lg shadow-violet-500/20 transition-opacity hover:opacity-90"
        >
          {t("viewLeaderboard")}
