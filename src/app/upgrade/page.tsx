    ...
    <div className="mx-auto min-h-[calc(100vh-4rem)] max-w-5xl px-4 py-16 md:px-8">
      ...
      <div className="mt-12 grid gap-6 md:grid-cols-2">
        ...
        <div className="rounded-[1.75rem] border border-slate-200 bg-white p-8 dark:border-white/10 dark:bg-white/5">
          ...
          <h2 className="mt-5 text-xl font-bold text-slate-900 dark:text-white">{t("free")}</h2>
          <div className="mt-2 flex items-baseline gap-1">
            <span className="text-4xl font-black text-slate-900 dark:text-white">$0</span>
            <span className="text-slate-500">{t("forever")}</span>
          </div>
        ...
        <div className="relative overflow-hidden rounded-[1.75rem] bg-gradient-to-br from-violet-600 via-fuchsia-500 to-cyan-500 p-px shadow-2xl shadow-violet-500/20">
          <div className="rounded-[1.7rem] bg-slate-900 p-8 dark:bg-slate-950">
            ...
            <h2 className="mt-5 text-xl font-bold text-white">{t("pro")}</h2>
            <div className="mt-2 flex items-baseline gap-1">
              <span className="text-4xl font-black text-white">$5.99</span>
              <span className="text-white/60">{t("oneTimeLower")}</span>
            </div>
            ...
            <label className="mt-6 flex items-start gap-3 rounded-2xl border border-white/10 bg-white/5 p-3 text-xs text-white/70">
              <input
                type="checkbox"
              ...
              <span>
                {t("acceptPrefix")}{" "}
                <a href="/cgu-cgv" target="_blank" className="underline underline-offset-2">
                  {t("termsOfSale")}
                </a>{" "}
                {t("waiverText")}
              </span>
            ...
            <Button
              ...
              className="mt-4 w-full bg-gradient-to-r from-violet-500 to-cyan-500 text-white hover:opacity-90 disabled:opacity-50"
            >
              {loading ? t("redirecting") : t("upgradeToProArrow")}
            </Button>
          </div>
        </div>
      </div>
      ...
      <div className="mt-10 flex flex-wrap items-center justify-center gap-6 text-sm text-slate-400">
        <div className="flex items-center gap-2"><BarChart3 className="h-4 w-4" /> {t("securePayment")}</div>
        <div className="flex items-center gap-2"><RotateCcw className="h-4 w-4" /> {t("refundPolicy")}</div>
        <div className="flex items-center gap-2"><Check className="h-4 w-4" /> {t("instantActivation")}</div>
      </div>
    </div>
  );
}
